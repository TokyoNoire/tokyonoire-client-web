...
const gameModules = [ {_id:231, typeOfModule: "question", title: "Akika Spotted", question: "", answer:"", image:"", description: "The last time she was seen was sitting on an unusual bench near the Mori Tower, in Roppongi.", locationCoordinates: [35.659439, 139.728384]}, {_id: 312, typeOfModule: "location", title: "The Heiress on the Loose", question: "", answer:"", image:"", description: "The last time she was seen was sitting on an unusual bench near the Mori Tower, in Roppongi.", locationCoordinates: [35.659439, 139.728384]}];
...
function SortableItem({ id, children }: SortableItemProps) {
  ...
  return (
    ...
    <div  className="self-center" ref={setNodeRef} style={style} {...attributes} {...listeners}>
      {children}
    </div>
  );
}
...
const DragAndDropEditor: FC = (): ReactElement => {
  const [state, setState] = useState<{id:number, title:string, moduleId: number, typeOfModule:string}[]>(contents);
          ...
          {state.map((item) => (
            <SortableItem key={item.moduleId} id={item.id}>
              <div className="h-32 mt-3 text-center rounded-md flexCenterDiv bg-darkGrey w-52">
                <h1 className="mb-3 ml-5 text-lg text-left text-black uppercase font-heading">{item.typeOfModule}</h1>
