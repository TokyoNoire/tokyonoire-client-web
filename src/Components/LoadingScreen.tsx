...
const LoadingScreen = (props: props): ReactElement => {
  ...
  useEffect(() => {
    if (!mounted) {
      setHint(
        LoadingScreenHints[
          Math.floor(Math.random() * LoadingScreenHints.length)
        ]
      );
    }
    setMounted(true);
  ...
  return Logo ? (
    <FadeDiv show={show} duration={fadeDuration}>
      <div className="absolute z-50 flex items-center justify-center w-screen h-screen flex-column loader-bg">
        ...
        <span
          className="absolute text-center top-128 display-linebreak"
          style={{ maxWidth: "90%" }}
        >
          {hint}
        </span>
